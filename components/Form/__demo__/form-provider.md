    ...
    <div>
      <Form.Provider
        ...
        onFormSubmit={(name, values, info) => {
          ...
          if (name === 'modalForm') {
            info.forms.searchForm.setFieldsValue({ email: values.email })
            setVisible(false);
          }
          Message.info({
            icon: <span></span>,
      ...
      >
        <Form id="searchForm" layout="vertical">
          <Grid.Row gutter={24}>
            ...
            <Grid.Col span={8}>
              <Form.Item label="Gender" field="gender">
                <Select placeholder="select gender" options={['All', 'Female', 'Male', 'Unknown']}/>
              </Form.Item>
            </Grid.Col>
            <Grid.Col span={8}>
          ...
          <Space>
            ...
            <Button onClick={() => { setVisible(true) }}>Add filter</Button>
          </Space>
        </Form>
        ...
        <Grid.Row justify="space-between" align="center">
          <Typography.Text style={{fontSize: 18}} bold>Result</Typography.Text>
          <RefreshForm />
        </Grid.Row>
        <br/>
        {visible && <ModalForm onCancel={() => {
          setVisible(false)
        }} />}
      </Form.Provider>
      <Table
