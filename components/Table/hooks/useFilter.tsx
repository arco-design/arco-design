...
function getCurrentFilters<T extends unknown>(flattenColumns: InternalColumnProps[]) {
  const currentFilters: FilterType<T> = {};
...
export default function useFilter<T extends unknown>(
  ...
  const currentFilters = getCurrentFilters<T>(flattenColumns);
