...
describe('Progress', () => {
  it('render correctly ', () => {
    ...
    expect(wrapper.find('.arco-progress-line-inner').at(0).props().style.width).toBe('20%');
  });

  it('render correctly when width props is string', () => {
    const wrapper = mount(<Progress percent={10} width="100%" />);
    expect(wrapper.find('.arco-progress').at(0).props().style.width).toBe('100%');
  });

  it('render correctly when width props is number', () => {
    const wrapper = mount(<Progress percent={10} width={200} />);
    expect(wrapper.find('.arco-progress').at(0).props().style.width).toBe(200);
  });
  ...
  it('render correctly when trailColor props is exist & type is circle', () => {
    ...
    expect(wrapper.find('.arco-progress-circle-mask').getDOMNode().getAttribute('style')).toBe(
      'stroke: rgb(0,0,0);'
    );
  });
});
